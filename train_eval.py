...
def predict_match(home_team, away_team, model_home, model_away, team_memory, elo, competition="Friendly", is_neutral=0):
    ...
    for h in range(6):
        for a in range(6):
            ...
            if prob > best_score_prob[0]:
                best_score_prob[0] = prob
                best_score [0]= (h, a)
            elif prob > best_score_prob[1]:
                best_score_prob[1] = prob
                best_score [1]= (h, a)
    ...
    print(f"Most Likely Result : {best_score[0][0]} - {best_score[0][1]} second most likely: {best_score[1][0]} - {best_score[1][1]}")
    print("-" * 45)
...
def compute_stats(history):
    ...
    # Exponential time-decay factor (newer games are weighted higher)
    gamma = 0.95
    ...
    for i, match in enumerate(history):
        ...
        # Get opponent Elo (fall back to average 1500 if not present)
        opp_elo = match.get("opponent_elo", 1500.0)
        elo_multiplier = opp_elo / 1500.0

        # Adjust scored/conceded goals based on opponent ELO rating
        adj_scored = match["scored"] * elo_multiplier
        adj_conceded = match["conceded"] / max(0.1, elo_multiplier)

        goals += adj_scored * w
        conceded += adj_conceded * w
    ...
    # Scale stats to the length of the history to preserve baseline feature scaling
...
tournaments_series = pd.Series(tournaments)

X_train, X_test, y_home_train, y_home_test, y_away_train, y_away_test, y_res_train, y_res_test, tournaments_train, tournaments_test = train_test_split(
    X, y_home, y_away, y_result, tournaments_series,
    test_size=0.2,
    shuffle=False
)
